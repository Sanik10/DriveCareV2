// path: apps/backend/src/modules/services/services/services-data.service.ts
...
export class ServicesDataService {
  ...
  async findWithFilters(filter: ServicesFilter): Promise<[Service[], number]> {
    ...
    if (filter.search) {
      query.andWhere('(service.name ILIKE :search OR service.description ILIKE :search)', {
  ...
  async update(id: string, dto: UpdateServiceDto): Promise<Service> {
    // Mass-assignment safe: whitelist fields
    const allowed: Partial<UpdateServiceDto> = {
      name: dto.name,
    ...
    await this.serviceRepository.update(id, allowed);
  ...
  async bulkUpdate(serviceIds: string[], updates: Partial<UpdateServiceDto>): Promise<number> {
    ...
    const result = await this.serviceRepository.update(serviceIds, allowed);
  ...
  async getServicesStats(companyId: string): Promise<ServiceStats> {
    ...
    const categoryStats = await this.serviceRepository
      ...
      .select(['service.categoryId as categoryId', 'category.name as categoryName', 'COUNT(*) as count'])
      .where('service.companyId = :companyId', { companyId })
  ...
  async findByPriceRange(companyId: string, minPrice: number, maxPrice: number): Promise<Service[]> {
    return this.serviceRepository
      .createQueryBuilder('service')
      .where('service.companyId = :companyId', { companyId })
      .andWhere('service.isActive = true')
      .andWhere('service.price BETWEEN :minPrice AND :maxPrice', { minPrice, maxPrice })
      .orderBy('service.price', 'ASC')
      .getMany();
  }

  async findByDuration(companyId: string, maxDuration: number): Promise<Service[]> {
    return this.serviceRepository
      .createQueryBuilder('service')
      .where('service.companyId = :companyId', { companyId })
