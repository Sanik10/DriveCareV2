// src/modules/invoices/services/invoices-business.service.ts (ИСПРАВЛЕННАЯ ВЕРСИЯ)
import { Injectable, Logger } from '@nestjs/common';
import { InjectRepository } from '@nestjs/typeorm';
import { Repository } from 'typeorm';
import { Invoice, Order, Payment } from '../../../database/entities';
import { PaymentStatus } from '../../../database/entities/payment.entity'; // 🔥 ДОБАВЛЕН ИМПОРТ
...
export class InvoicesBusinessService {
  ...
  async createInvoiceForCompany(data: CreateInvoiceData, companyId: string, user: UserWithCompany): Promise<Invoice> {
    ...
    // 🔒 ИСПРАВЛЕНО: Проверка лимитов подписки - используем публичный метод для заказов
    ...
    // 📊 ИСПРАВЛЕНО: Audit логирование - используем простой метод log
  ...
  async createInvoiceFromOrder(orderId: string, companyId: string, user: UserWithCompany, options?: {
    ...
    const order = await this.orderRepository.findOne({
      ...
      relations: ['orderServices', 'orderParts'],
    });
    ...
    const invoiceData: CreateInvoiceData = {
      ...
      amount,
      dueDate,
      notes: options?.notes || `Счет за выполненные работы по заказу ${order.orderNumber}`,
    };

    // 🎯 Создание счета
    const invoice = await this.createInvoiceForCompany(invoiceData, companyId, user);

    // 📊 ИСПРАВЛЕНО: Дополнительный audit
  ...
  async changeInvoiceStatus(id: string, newStatus: InvoiceStatus, user: UserWithCompany): Promise<Invoice> {
    ...
    // 🔒 ИСПРАВЛЕНО: Проверка возможности изменения статуса с правильными типами
  ...
  async cancelInvoice(id: string, user: UserWithCompany): Promise<Invoice> {
    ...
    // 🔍 ИСПРАВЛЕНО: Проверка наличия платежей с правильным enum
    const payments = await this.paymentRepository.find({
      where: { invoiceId: id, status: PaymentStatus.PROCESSED },
    });
  ...
  async processPaymentReceived(invoiceId: string, paymentAmount: number, user: UserWithCompany): Promise<Invoice> {
    ...
    // 🧮 ИСПРАВЛЕНО: Расчет оплаченной суммы с правильным enum
    const currentPayments = await this.paymentRepository.find({
      where: { invoiceId, status: PaymentStatus.PROCESSED },
    });
  ...
  async processOverdueInvoices(companyId: string): Promise<{
    ...
    for (const invoice of overdueInvoices) {
      ...
      // 📊 ИСПРАВЛЕНО: Логирование просрочки
