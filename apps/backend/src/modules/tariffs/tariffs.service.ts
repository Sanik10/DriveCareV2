...
export class TariffsService {
  ...
  constructor(
    ...
    private readonly tariffsMapperService: TariffsMapperService, // 🔥 ДОБАВЛЕНО
  ) {}
  ...
  async create(createTariffDto: CreateTariffDto): Promise<TariffResponseDto> {
    ...
    return this.tariffsMapperService.mapToResponseDto(tariff); // 🔥 ИСПРАВЛЕНО: используем MapperService
  }
  ...
  async findAll(filter: TariffFilter = {}): Promise<PaginatedTariffsResult> {
    ...
    // 🔥 ИСПРАВЛЕНО: используем MapperService
    return {
  ...
  async findActive(): Promise<TariffResponseDto[]> {
    ...
    return this.tariffsMapperService.mapArrayToResponseDto(tariffs); // 🔥 ИСПРАВЛЕНО: используем MapperService
  }
  ...
  async update(id: string, updateTariffDto: UpdateTariffDto): Promise<TariffResponseDto> {
    ...
    return this.tariffsMapperService.mapToResponseDto(updatedTariff); // 🔥 ИСПРАВЛЕНО: используем MapperService
  }
  ...
  /**
   * Удаление тарифа (только для superadmin)
   */
  ...
  async compareTariffs(tariffIds: string[]): Promise<TariffResponseDto[]> {
    ...
    const tariffs = await Promise.all(
      tariffIds.map(id => this.tariffsValidationService.validateTariffExists(id))
    );
  ...
  /**
   * 🔥 УЛУЧШЕНО: Получение базовой информации о тарифе (для других модулей)
   */
  async getTariffInfo(id: string): Promise<{
    id: string;
    name: string;
    priceMonthly: number;
    priceYearly: number;
    ...
    return this.tariffsMapperService.mapToBasicInfo(tariff); // 🔥 ИСПРАВЛЕНО: используем MapperService
  }
  ...
  /**
   * 🔥 НОВОЕ: Получение данных для сравнения тарифов
   */
  async getComparisonData(tariffIds: string[]): Promise<{
    id: string;
    name: string;
    priceMonthly: number;
    priceYearly: number;
  ...
  /**
   * 🔥 НОВОЕ: Получение статистики по тарифам
   */
  async getStats(): Promise<{
    ...
    // TODO: Добавить реальный подсчет подписок когда будут связи между модулями
  ...
  /**
   * 🔥 НОВОЕ: Получение опций для селектов (для других модулей)
   */
  async getSelectOptions(): Promise<{
    ...
    return activeTariffs.map(tariff => this.tariffsMapperService.mapToSelectOption(tariff));
  }

  // 🔥 УДАЛЕНО: Приватные методы mapToResponseDto и isRecommendedTariff
  // Теперь вся логика маппинга в TariffsMapperService
}
