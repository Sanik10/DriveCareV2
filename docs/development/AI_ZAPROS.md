...
### 📋 **Как запустить Claude в режиме DriveCare Developer:**

1. **Скопируй весь промпт выше** и вставь в начало новой сессии
2. **Опиши задачу** кратко
3. **Жди запроса файлов** - Claude всегда спросит нужные файлы перед работой
4. **Предоставь файлы** по списку
5. **Получи качественное решение** с пошаговыми инструкциями

### 🎯 **Пример запуска:**
```
[ВСТАВИТЬ ПРОМПТ ВЫШЕ]

Задача: Нужно создать новый модуль Customers для управления клиентами автосервиса.

[Дождаться списка нужных файлов от Claude]
```

### 📝 **Claude всегда будет спрашивать файлы вот так:**
```
📋 МНЕ НУЖНЫ файлы для создания модуля Customers:

# Архитектура
src/app.module.ts
src/modules/companies/companies.module.ts (для примера)

# Entities
src/database/entities/index.ts
src/database/entities/customer.entity.ts (если есть)

# Security
src/common/guards/company-ownership.guard.ts
src/common/decorators/resource.decorator.ts

Приложи эти файлы и я создам качественный модуль! 🚀
```

---

**🎯 Этот промпт настроит Claude как опытного DriveCare разработчика, который знает все архитектурные решения, следует security-first принципам и создает enterprise-качественный код!** 🚀

**Сохрани этот промпт и используй в каждой новой сессии для максимального качества! 🛡️**