...
Обновлено: текущей сессией (Appointments: асинхронные селекты клиент/авто/мастер/услуги; Smart‑Schedule диалог с автоподстановкой; диалоги Complete/Rating вместо prompt; мини‑виджет трекинга с автопуллингом; нормализация статусов/приоритетов в UI; check‑availability поддерживает timeRange; Services/Users API‑клиенты — нормализация ответов; фикс даты в деталях инвойса)
...
- API‑клиенты:
  - usersAPI/search — нормализация форм ответа (items/users/array).
  - servicesAPI/search — поддержка {items} и {data,pagination}; duration/durationMinutes.
...
- Appointments:
  - Асинхронные селекты: механик/клиент/авто/услуги (debounce, safe‑fetch, AbortController).
  - Smart‑Schedule: полноценный диалог подбора слотов с автоподстановкой.
  - Действия: Complete/Rating переведены на унифицированные Dialog‑формы.
  - Деталка: мини‑виджет трекинга с автопуллингом, проверка статуса.
  - API: check‑availability теперь принимает timeRange; smart‑schedule возвращает estimatedWaitTime/nextAvailableDate.
  - UI устойчив к регистру статусов/приоритетов (case‑insensitive).
- API‑клиенты:
  - servicesAPI/usersAPI — нормализация разнородных ответов (items | data+pagination | users | array).
- Invoices/Payments:
  - «Платежи по счёту» и UI‑роль‑гейтинг Refund — на месте.
  - Фикс типизации даты в деталях инвойса (сборка Next OK).

Что ещё требует доработок:
- Appointments:
  - Smart‑Schedule v2: интеграция с рабочими графиками и исключениями (work‑schedules) для точных слотов.
  - Обогащение деталки записи — показать услуги с ценами/длительностью.
  - Единый форматтер денег/времени, явная TZ‑нормализация в формах datetime‑local.
- Payment‑Methods:
  - Bulk‑операции (admin‑only) — добавить UI.
- Payments:
  - Аналитика /payments/analytics/statistics — экран с графиками и фильтрами.
- Полировка:
  - Микро‑скелетоны/плейсхолдеры в деталях записей.
  - Единый FeatureGuard для UI‑гейтинга по ролям.
...
Frontend
- npm run lint — линт (устраним ворнинги в отдельной задаче)
